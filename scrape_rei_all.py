...
def main(suffix=None):
    ...
    # Open postgresql connection, insert rows, close
        ...
        db_conn.insert_rei_all(df_items, items_table_name, run_id, dt)
        db_conn.insert_rei_all(df_page, page_table_name, run_id, dt)
        db_conn.close()
        
        offset += limit # increment to next page
        item_count = df_page['count'].item() # Assign item_count
